...
export default function ResetPasswordModal({ open, onClose, userId, userName, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.put(`/admin/users/${userId}/reset-password`, { newPassword: form.next });
      setSuccess(true);
      ...
      >
        ...
        {success ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-700">Password reset successfully</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <p className="text-sm text-gray-500">
              Set a new password for <span className="font-semibold text-gray-700">{userName}</span>. Share it with them — they can change it later from their own account.
            </p>
            ...
            <div className="flex justify-end gap-3 pt-1">
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-2 px-5 py-2.5 text-white rounded-lg text-sm font-semibold bg-brand-500 hover:bg-brand-600 transition-colors disabled:opacity-50"
              >
