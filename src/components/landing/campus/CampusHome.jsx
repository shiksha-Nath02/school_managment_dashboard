// =============================================================================
// CAMPUS LAYOUT — Home tab (hero overview + latest news)
// =============================================================================
// A dedicated hero tuned for the in-flow sticky header (no fixed-header offset).
// CTAs switch tabs via onNavigate rather than scrolling to anchors.
// =============================================================================
...
export default function CampusHome({ onNavigate }) {
    ...
    <>
      <section className="relative">
        <div className="relative min-h-[78vh] flex items-center justify-center overflow-hidden">
          {hero.image ? (
            <img src={hero.image} alt={site.name} className="absolute inset-0 w-full h-full object-cover animate-ken-burns" />
          ) : (
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center px-6 py-20">
            <div className="flex justify-center mb-6 animate-fade-up animate-start">
              {site.logo ? (
                <img src={site.logo} alt={site.name} className="w-20 h-20 rounded-full object-cover ring-4 ring-gold/60" />
              ) : (
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-6 md:px-12 -mt-16">
          <div className="grid grid-cols-2 md:grid-cols-4 bg-white shadow-card rounded-md overflow-hidden border border-gray-100">
            {stats.map((s, i) => (
              <div
                key={s.label}
                className={`p-7 text-center ${i !== stats.length - 1 ? 'md:border-r border-gray-100' : ''} ${
                  i < 2 ? 'border-b md:border-b-0 border-gray-100' : ''
                }`}
              >
                <CountUp value={s.value} className="block font-display text-3xl md:text-4xl font-extrabold text-brand-700" />
                <div className="text-xs uppercase tracking-wide text-gray-400 mt-1.5 font-semibold">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
